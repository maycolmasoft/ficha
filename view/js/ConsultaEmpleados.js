...
function search(_page = 1){
	...
	$.ajax({
		...
		url:"index.php?controller=Empleados&action=search",
		type:"POST",
...
$.ajax({
	beforeSend:function(){},
	url:"index.php?controller=Empleados&action=cargaTipoFicha",
	type:"POST",
	dataType:"json",
	data:null
}).done(function(datos){		
...
$("#frm_ficha").on("submit",function(event){
   ...
   if($mod_tip_id > 0) {  
	...
	$.ajax({
		beforeSend:function(){$("#divLoaderPage").addClass("loader")},
		url:"index.php?controller=Empleados&action=IniciarFicha",
		type:"POST",
		dataType:"json",
		data:parametros
